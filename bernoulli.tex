...
$$
for each $n \in \bN$. For instance, we have 
$f_0(\ttx) = f_1(\ttx) = 0$,
$f_2(\ttx) = \ttx^2$,
...
\begin{proof}
  Let $x \in \left]- 2, 0 \right[ \cup \gtint{0}$ and let $n \in \bN + 1$.
  Put $y = 1 + x$.
  Clearly,
  $x \in \left]- 2, 0 \right[$ implies $1 > \abs{y} \ge \abs{y}^n \ge y^n$
  and
  $x \in \gtint{0}$ implies $1 < y \le y^n$,
  so $x$ and $y^n - 1$ have the same sign.
  % Clearly, $x \in \left]- 2, 0 \right[$ implies $\abs{y}^n < 1$ and
  % $x \in \gtint{0}$ implies ${(1 + x)}^n > 1$,
  % so ${(1 + x)}^n - 1$ and $x$ have the same sign.
Besides, straightforward computations yield 
   $$
   f_{n + 1}(\ttx) - f_n(\ttx)
   = x  \left( {(1 + \ttx)}^n - 1 \right) \,,
   $$
   whence 
   $$f_{n + 1}(x) - f_n(x) = x (y^n - 1) >  0\,,
   $$
   as desired.
\end{proof} 
 ...
 \begin{proof}
   First, assume $x \ge - 1$.
   Then, ${(1 + x)}^m - 1$ has the same sign as $x$ for every $m \in \bN + 1$.
   Besides, straightforward computations yield
   $$
   f_{m + 1}(\ttx) - f_m(\ttx) =  \left( {(1 + \ttx)}^m - 1 \right) \ttx 
   $$
   for every $m \in \bN$.
   Therefore, the sequence $\left( f_m(x) \right)_{m \in \bN + 1}$ is increasing.
   It follows $f_n(x) > f_1(x) =  0$, as desired.
   Second, assume $n \in 2 \bN + 2$ and $x \le - 1$.
 ...
 \begin{theorem}
  ...
  \ttx^n - \tty^n
   =
   (\ttx - \tty) \sum_{k = 0}^{n - 1} \ttx^{n - 1 - k}\tty^k 
   $$
