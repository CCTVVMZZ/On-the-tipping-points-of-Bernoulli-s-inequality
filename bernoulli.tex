...
Straightforward computations yield the following recurrence relation which is used as a lemma:
 ...
 \begin{proof}
  ...
  % For every $n \in 2 \bN + 3$, $t_n \le u_{n - 2}$ is equivalent to $f_n(u_{n - 2}) \ge 0$.
   ...
   Since $u_{n - 2}$ not greater than $u_n$, $t_n$ is not greater than $u_n$,
   and thus $f_n(u_n)$ is non-negative.
   Therefore,  
...
\begin{theorem}
  For every $n \in 2 \bN + 3$, $t_n$ is greater than $- 2 - $.
\end{theorem}

\begin{proof}
  Put
  $$
  g_n (\ttx) = {(1 + \ttx)}^n + 1 - 2 n - n x = f_n(\ttx) - 2 n + 2
  $$
  for every $n \in \bN$.
  Since 
  $$
  f_n(- 2 - x)  = {(- 1)}^{n + 1}  {( 1 + \ttx)}^n - 1 + 2 n + n \ttx 
  $$
  $n \in 2 \bN + 1 $ implies $g_n(\ttx) = - f_n(- 2 - \ttx)$.

  $$
  {(1 + x)}^n \ge 1 + n x + \frac{(n - 1)n}{2}  x^2
  $$
  $$
  g_n(x) \ge 2 - 2 n + \frac{(n - 1) n}{2} x^2 
  $$
  $g_n(x) = 0$ is equivalent to 
\end{proof} 
Let $n \in \bN$.
Put
$$
g_n(\ttx) = {(1 + x)}^n + 1  - n \ttx - 2 n
$$

$$
f_n(- 2 - \ttx) = {(- 1)}^n {(1 + \ttx)}^n - 1 + n \ttx + 2 n
  $$
  ...
  $$
  g_n(x) \ge  \frac{n(n - 1)}{2} x^2 - 2(n + 1) 
  $$
